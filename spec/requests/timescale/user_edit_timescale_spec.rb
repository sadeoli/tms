...
describe 'Usuário edita um prazo' do
  it 'e é regular' do
    ...
    timescale = Timescale.create!(min_distance: 1, max_distance: 10, deadline: 24,
                                  transportation_modal:)
    user = User.create!(email: 'usuario@sistemadefrete.com.br', password: 'password', access_group: :regular)

    # Act
    login_as(user)
    patch(transportation_modal_timescale_path(transportation_modal.id, timescale.id),
          params: { timescale: { deadline: 48 } })
